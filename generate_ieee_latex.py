...
def _sec_algorithms() -> str:
    ...
    alg2 = build_algorithm_env(
        ...
        steps=[
            ...
            "    IF $p$ is currently in cooldown period THEN continue to next.",
            ...
            "        ENDIF",
            "        IF 401 Unauthorized error: set 300s cooldown; break model loop.",
            "        IF 429 Rate Limit error: rotate model index; sleep 0.4s; retry.",
            "    ENDFOR",
    ...
    alg3 = build_algorithm_env(
        ...
        steps=[
            ...
            "IF $H_{16} \\in \\mathcal{H}$ THEN RETURN \\textit{False} (duplicate rejected).",
            "ELSE insert $H_{16}$ into $\\mathcal{H}$; RETURN \\textit{True} (question accepted).",
            "ENDIF",
        ],
    )
    ...
    alg4 = build_algorithm_env(
        ...
        steps=[
            ...
            "IF $S \\geq 85$: status = HONORABLE.",
            "ELSE IF $50 \\leq S < 85$: status = SUSPICIOUS.",
            "ELSE: status = HIGH CHEATING RISK.",
            "ENDIF",
            "RETURN result dict.",
        ],
    )
    ...
    alg5 = build_algorithm_env(
        ...
        steps=[
            ...
            "    IF $H \\notin$ question bank: append $q$ with timestamp metadata.",
            "    Increment question type and topic frequency maps.",
            "ENDFOR",
            "FOR each topic $t$ in $\\mathcal{T}$:",
            "    IF $t \\notin$ topic store: initialize topic node.",
